...
def train(opt, device):
    ...
    # Dataloaders
    data_dir = data  # e.g. ../dataset_1

    train_dir = data_dir / "train"
    val_dir   = data_dir / "val"
    test_dir  = data_dir / "test"

    nc = len([x for x in train_dir.glob("*") if x.is_dir()])  # number of classes
    ...
    train_correct, train_total = 0, 0 # ADDED
    ...
    for epoch in range(epochs):  # loop over the dataset multiple times
        ...
        train_correct, train_total = 0, 0  # NEW: reset per epoch
        ...
        for i, (images, labels) in pbar:  # progress bar
            ...
            with amp.autocast(enabled=cuda):  # stability issues when enabled
                logits = model(images)               # CHANGED: keep logits
            ...
            if RANK in {-1, 0}:
                ...
                # Test
                # Validation + test at end of epoch
                if i == len(pbar) - 1:  # last batch
                    # validation on val split
                    val_top1, val_top5, vloss = validate.run(
                        model=ema.ema, dataloader=valloader, criterion=criterion, pbar=pbar
                    )
                    # test on test split
                    test_top1, test_top5, test_loss = validate.run(
                        model=ema.ema, dataloader=testloader, criterion=criterion, pbar=None
                    )

                    fitness = val_top1  # use val accuracy as fitness
        ...
        if RANK in {-1, 0}:
            ...
            metrics = {
                ...
                "metrics/val_accuracy_top1": val_top1,
                "metrics/val_accuracy_top5": val_top5,
                "metrics/test_accuracy_top1": test_top1,
                "metrics/test_accuracy_top5": test_top5,
